...
et = pd.read_csv("subject-30_TOBII.csv")
beh = pd.read_csv("subject-30.csv")
op = pd.DataFrame()
...
#et.iloc[2:500,["CBcond","Gamble","domain","X_location","risk_selected","response_time"]]
...
def get_trials_indexes(df):
    ...
    for i in df.index:
        val = str(et.at[i,"Event"]) # The value at the current cell
        val2 = str(et.at[i,"TimeStamp"])
        val3 = str(et.at[i,"GazePointX"])
        val4 = str(et.at[i,"GazePointY"])
        tL = [val,val2,val3,val4]
        
        if any("gamble" in x for x in tL):   
            #print(tL)
...
def get_trial_from_indexes(f):
    ...
    t = pd.DataFrame(et.iloc[first_gamble_index:last_gamble_index,["TimeStamp","Event","GazePointX","GazePointY"]])
    print(t)
    return t
 


def temp_print(temp_counter,limit,duration,x_loc):
    if temp_counter <= limit: 
        print(temp_counter,duration,x_loc)
...
def main():  
    ...
    for f in trials: 
        ...
        #print("Index: %d Gamble: %d Bad: %d Total: %d P: %f Q: %f X: %f Y:%f"%(c,gamble_nr,len(temp_fix),len(fixations),len(AOI_p),len(AOI_q),len(AOI_x),len(AOI_y)))     
        #print("The excluded ones: ...",temp_fix)
